...
def main():
    ...
    if command == "decode":
    ...
    elif command == "download": #most of the logic for downloading entire .torrent file will be the same as downloading a single piece of a .torrent file
        torrentFile = sys.argv[4]
        torrentData = torrentReader(torrentFile)
        tracker = torrentData[b"announce"].decode() #the tracker URL with the ip addresses
        length = torrentData[b"info"][b"length"] #it's a nested dictionary to begin with, so we need two keys to get the length
        ...
        #established tcp handshake
            ...
            numOfPieces = (length + pieceLength -1)//pieceLength #get number of pieces in file, accounting for last piece maybe being shorter
            totalFile = bytearray() #empty array that'll hold the total file data

            for currentPieceIndex in range(numOfPieces): #go through every piece for all info in the .torrent file
                if int(currentPieceIndex) == numOfPieces-1: #if it's the last one
                    currentPieceLength = length - (int(currentPieceIndex) * pieceLength) #calculate length of last piece which is likely to be smaller thatn pieceLength which is how long the other pieces before this are likely to be
                else:
                    currentPieceLength = pieceLength #default to pieceLength if not last piece

                pieceData = bytearray() #bytearray for piece data that will eventually be combined into totalFile
                subPieceSize = 16384

                for i in range(0, currentPieceLength, subPieceSize):
                    currentSubPieceLength = min(subPieceSize, currentPieceLength-i) #if last piece smaller than 16384, we can still use it

                    requestToSend = struct.pack(">IBIII", 13, 6, currentPieceIndex, i, currentSubPieceLength) #these are the parameters of our request message
                    #1. 13 for length excluding prefix, 2. 6 for message id being 6, 3. currentPieceIndex for what piece we want, 4. i represents our offset as we move through the piece, 5.currentSubPieceLength for how much data to take from our current offset(usually 16384 unless the last subPiece is smaller)
                    clientSocket.sendall(requestToSend) #request for subpiece sent
                    subPiecePrefix = clientSocket.recv(4) #receive 4byte prefix
                    ...
                    subPiece = b"" #this is where the subPiece data will be stored until added to pieceData bytearray
                    while len(subPiece) < subPieceLength:
                        packet = clientSocket.recv(subPieceLength - len(subPiece)) 
                        if not packet:
                            break
                        subPiece += packet #add data to subPiece
                    #while loop gets data until we've matched the length the subPiece is supposed to be

                    if subPiece and subPiece[0] == 7:
                        subPiecePayload = subPiece[9:] #extract payload which is after the prefix
                        pieceData.extend(subPiecePayload) #payload is actual data, so that's what's added, excluding the byte headers and prefixes
                    #now have continous array of bytes representing data in the piece
                
                hashOfData = hashlib.sha1(pieceData).digest() #turn piece into hash
                allHashes = allHashes
                amountForward = int(currentPieceIndex) * 20 #this is where the piece specific hash starts in allHashes
                expectedHash = allHashes[amountForward : amountForward+20] #the hash of the piece we were given
                if (hashOfData == expectedHash): #final check where we compare our data's hash to the piece's hash that we were already given
                    totalFile.extend(pieceData) #if hashes match, add piece to totalFile data, ensuring in the end, all our pieces for the file are correct
                else:
                    raise ValueError(f"Hash mismatch, hashes do not match")
                    
        with open(outputPath, "wb") as fileToWriteTo:
            fileToWriteTo.write(totalFile) #add file data to correct location
        print(f"downloaded .torrent file to {outputPath} successfully", file=sys.stderr)
